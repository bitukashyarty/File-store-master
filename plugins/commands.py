...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('JOIN backup', url='https://t.me/+Frfvg1Vy_spiNTM1')
            ]]
        if CLONE_MODE == True:
...
# Ask Doubt on telegram @KingVJ01
    ...
    if data.split("-", 1)[0] == "verify":
        ...
        if AUTO_DELETE_MODE == True:
            k = await client.send_message(chat_id = message.from_user.id, text=f"<b>❗️IMPORTANT❗️️</b>\n\nThis Video will be deleted in <b>{AUTO_DELETE} minutes</b>")
...
# Ask Doubt on telegram @KingVJ01
        ...
        buttons = [
            InlineKeyboardButton('JOIN BACKUP', url='https://t.me/backcatmaterial')
            ]
